...
function getImagesByProductId(productId) {
  return productImagesCache.filter((img) => Number(img.product_id) === Number(productId)).sort((a, b) => Number(a.sort_order || 0) - Number(b.sort_order || 0));
}
...
function renderProducts() {
  ...
  filtered.forEach((product) => {
    const extraImages = getImagesByProductId(product.id);
    ...
    card.innerHTML = `
      ...
      <div class="product-admin-preview">
        <div class="product-admin-image">${product.image_url ? `<img src="${product.image_url}" alt="${product.name}">` : "<span>Nessuna foto</span>"}</div>
        <div class="order-meta">
          ...
          <div><strong>Gallery:</strong> ${extraImages.length} immagini</div>
...
async function loadCategories() {
  const { data, error } = await supabaseClient.from("categories").select("*").order("sort_order", { ascending: true }).order("id", { ascending: true });
  if (error) { adminStatus.textContent = `Errore categorie: ${error.message}`; return; }
...
async function loadProductImages() {
  ...
  if (error) { adminStatus.textContent = `Errore immagini: ${error.message}`; return; }
  productImagesCache = data || [];
}

async function loadProducts() {
  productsList.innerHTML = "<p class='empty-message'>Caricamento prodotti...</p>";
  const { data, error } = await supabaseClient.from("products").select("*").order("sort_order", { ascending: true }).order("id", { ascending: false });
  if (error) { productsList.innerHTML = `<p class='empty-message'>Errore: ${error.message}</p>`; return; }
  productsCache = data || [];
  renderProducts();
}

function renderExistingImages(productId) {
  const images = getImagesByProductId(productId);
  if (!images.length) {
    existingProductImages.innerHTML = "<p class='empty-message'>Nessuna immagine aggiuntiva caricata.</p>";
    return;
  }
  existingProductImages.innerHTML = images.map((img) => `
    <div class="gallery-admin-item">
      <img src="${img.image_url}" alt="Immagine prodotto">
      <button class="gallery-admin-remove" type="button" data-remove-image="${img.id}">✕</button>
    </div>`).join("");
}
...
function fillForm(productId) {
  ...
  renderExistingImages(product.id);
...
async function uploadImageToStorage(file) {
  ...
  const { error } = await supabaseClient.storage.from(STORAGE_BUCKET).upload(filePath, file, { upsert: false });
...
async function uploadCoverIfNeeded() {
  ...
  return uploadImageToStorage(imageInput.files[0]);
}
...
async function uploadGalleryImages(productId) {
  ...
  let sortOrderBase = currentImages.length;
  for (const file of galleryInput.files) {
    const image_url = await uploadImageToStorage(file);
    const { error } = await supabaseClient.from("product_images").insert([{ product_id: productId, image_url, sort_order: sortOrderBase }]);
    if (error) throw error;
    sortOrderBase += 1;
  }
}
...
async function saveProduct(event) {
  ...
  try {
    ...
    const image_url = await uploadCoverIfNeeded();
    const slug = slugify(name);

    const payload = { name, slug, category_id, price, price_on_request, description, sort_order, featured, active, image_url };

    let response;
    let productId = id ? Number(id) : null;
    if (id) response = await supabaseClient.from("products").update(payload).eq("id", productId);
    else {
      response = await supabaseClient.from("products").insert([payload]).select("id").single();
...
async function removeProductImage(imageId) {
  const confirmed = confirm("Vuoi eliminare questa immagine?");
  if (!confirmed) return;
  const { error } = await supabaseClient.from("product_images").delete().eq("id", imageId);
  if (error) { adminStatus.textContent = `Errore eliminazione immagine: ${error.message}`; return; }
  await loadProductImages();
  const currentProductId = document.getElementById("productId").value;
  if (currentProductId) renderExistingImages(Number(currentProductId));
  adminStatus.textContent = "Immagine eliminata.";
}

async function toggleProduct(productId) {
  const product = productsCache.find((p) => p.id === Number(productId));
  if (!product) return;
  const { error } = await supabaseClient.from("products").update({ active: !product.active }).eq("id", product.id);
  if (error) { adminStatus.textContent = `Errore update: ${error.message}`; return; }
...
async function duplicateProduct(productId) {
  ...
  if (error) { adminStatus.textContent = `Errore duplicazione: ${error.message}`; return; }

  const sourceImages = getImagesByProductId(product.id);
  for (const img of sourceImages) {
    await supabaseClient.from("product_images").insert([{ product_id: data.id, image_url: img.image_url, sort_order: img.sort_order || 0 }]);
  }
...
async function deleteProduct(productId) {
  ...
  if (error) { adminStatus.textContent = `Errore delete: ${error.message}`; return; }
...
loginForm.addEventListener("submit", async (event) => {
  ...
  if (error) { adminStatus.textContent = `Errore login: ${error.message}`; return; }
...
refreshBtn.addEventListener("click", async () => { await loadCategories(); await loadProductImages(); loadProducts(); });
logoutBtn.addEventListener("click", async () => { await supabaseClient.auth.signOut(); adminStatus.textContent = "Logout effettuato."; productsList.innerHTML = "<p class='empty-message'>Effettua il login per vedere i prodotti.</p>"; });
...
document.addEventListener("click", (event) => {
  ...
  const removeImageId = event.target.getAttribute("data-remove-image");
  ...
  if (removeImageId) removeProductImage(removeImageId);
});
